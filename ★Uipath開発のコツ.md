...
UI操作は**「遅く、不安定」**になりがちだからです。
*   画面デザインの変更で簡単に壊れる
*   処理に時間がかかる
*   PC操作中は実行できない（バックグラウンド実行不可）

### UI操作の代替案とは？

**「画面」ではなく「裏側のシステム」を直接操作する**ことです。これにより、**高速で安定したバックグラウンド実行可能**なロボットになります。

| 対象アプリ | UI操作の代替案 |
| :--- | :--- |
| **Webサイト** | **API連携 (`HTTP Request`)**<br>最も高速・安定。Webサービスの裏口から直接データをやり取りする。 |
| **Excel** | **Workbook系アクティビティ**<br>Excelアプリを起動せず、ファイル自体を直接読み書きする。高速。 |
| **デスクトップアプリ (SAPなど)** | **専用APIやBAPI連携**<br>アプリケーションが提供する公式の連携機能を使う。最速・安全。 |
| **汎用アプリ** | **データベース直接接続**<br>アプリを介さず、裏側のDBに直接SQLでアクセスする。大量データに強い。 |

---

### どうしてもUI操作が必要な場合

アクティビティのプロパティで**入力メソッド**を `Hardware Events`（デフォルト）から **`Simulate`（シミュレート）**に変更します。これにより、処理が高速化し、バックグラウンドでの実行が可能になる場合があります。
---
